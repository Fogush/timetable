...
class GantrySplitmenuLayout extends AbstractRokMenuLayout
{
    ...
    public function stageHeader()
    {
        ...
        $gantry->addLess('menu.less', 'menu.css', 1, array('headerstyle'=>$gantry->get('headerstyle','dark'), 'menuHoverColor'=>$gantry->get('main-global'), 'main-global'=>$gantry->get('main-global')));

        // no media queries for IE8 so we compile and load the hovers
        if ($gantry->browser->name == 'ie' && $gantry->browser->shortver < 9){
            $gantry->addLess('menu-hovers.less', 'menu-hovers.css', 1, array('headerstyle'=>$gantry->get('headerstyle','dark'), 'menuHoverColor'=>$gantry->get('main-global'), 'main-global'=>$gantry->get('main-global')));
        }
    }
