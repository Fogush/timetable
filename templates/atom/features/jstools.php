...
class GantryFeatureJSTools extends GantryFeature {
	...
	function init() {
	 ...
	 // load jQuery, if not loaded before
		...
		// load blockquote
		$document->addScript($template_dir."/js/jquery.quovolver.js");	
	
		//load powertip
		$document->addScript($template_dir."/js/jquery.powertip.js");		
		
		
		if($gantry->get('stickmenu') == 1) {// add custom javascript code
			$document->addScript($template_dir."/js/functions.php?template_dir=".urlencode($template_dir)."&amp;stickmenu=".$gantry->get('stickmenu'), 'text/javascript');
		}
		else
		{
			$document->addScript($template_dir."/js/functions.php");
		}
