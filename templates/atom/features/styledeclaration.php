...
class GantryFeatureStyleDeclaration extends GantryFeature {
	...
	function init() {
        ...
        // Logo
        $css = $this->buildLogo();
		 // Showcse and bottom position background colors
		$showcasecolor = new Color($gantry->get('showcasecolor'));
		$css .= '#rt-showcase  {background:' . $gantry->get('showcasecolor') . ';}';
		$css .= '#rt-showcase {background-color: '.$showcasecolor->lighten('4%').'; '.$this->_createGradient('top', $showcasecolor->lighten('4%'), '1', '0%', $showcasecolor->darken('9%'), '1', '100%').'}'."\n";
		$css .= '#rt-bottom  {background:' . $gantry->get('bottomcolor') . ';}';
		$css .= '#rt-footer  {background:' . $gantry->get('footercolor') . ';}';
